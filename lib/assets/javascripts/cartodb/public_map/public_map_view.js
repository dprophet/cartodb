...
module.exports = cdb.core.View.extend({
  ...
  _createVis: function() {
    ...
    //Switch the description, title if it's not in the right place
    // we want to display 'Title' followed by the 'description' in the map
    var overlays = this.options.vizdata.overlays;
    var iTitle = -1;
    var iDescription = -1;
    for(var i=0; i < overlays.length; ++i) {
      var overlay = overlays[i];
      if (overlay.type == 'header' && overlay.options.extra.show_description) {
        iDescription = i;
      } 
      else if (overlay.type == 'header' && overlay.options.extra.show_title) {
        iTitle = i;
      }
    }

    if (iTitle !== -1 && iDescription !== -1 && iTitle > iDescription) {
       var delTitles = overlays.splice(iTitle, 1);
       overlays.splice(iDescription, 0, delTitles[0]);
    }
