  ...
  SearchInput = Backbone.View.extend({
    ...
    _updateSearchIndex: function() {
      ...
      for (i = 0; i < numItems; ++i) {
          ...
          searchItems.push({ id: i, name: item.name, tags: item.tags });
      }
    ...
    _loadDatasetList: function() {
      ...
      .done(function(response) {
          ...
          for (i = 0; i < numResults; ++i) {
            result = results[i];
            self._allItems.push({ id: result.id, type: result.type, name: result.name_alias || result.display_name || result.name,
    ...
    _renderListItem: function(index, item, selected) {
      ...
      var startPos = item.name.toLowerCase().indexOf(query);
      if (startPos == -1) {
          name = item.name;
      } else {
