...
module.exports = cdb.core.View.extend({
  ...
  events: {
    ...
    'mouseenter': '_initInfoPopup',
    'click': '_selectDataset'
  },

  _infoTooltip: undefined,

  initialize: function() {
    this.user = this.options.user;
    this.router = this.options.router;
    this.template = cdb.templates.getTemplate('dashboard/views/datasets_table_item');
    this.infoTooltipTemplate = cdb.templates.getTemplate('dashboard/views/datasets_item_info_tooltip');
  ...
  _initInfoPopup: function(ev) {
    var self = this;
    if (this._infoTooltip === undefined) {
      this._infoTooltip = new cdb.common.TipsyTooltip({
        el: this.$('.js-item-info'),
        gravity: 'se',
        delayIn: 500,
        delayOut: 1000,
        html: true,
        title: function() {
          ...
          return self.infoTooltipTemplate({
            name: vis.get('display_name') || vis.get('table').name_alias || vis.get('name'),
            source: vis.get('source'),
            updated: moment(vis.get('updated_at')).format("MMM D, YYYY  h:mm a"),
            exportable: vis.get('exportable'),
            export_geom: vis.get('export_geom'),
          });
        }
      });
      this.addView(this._infoTooltip);
    }
  }
