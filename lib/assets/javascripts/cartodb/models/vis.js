  ...
  tableMetadata: function() {
    if (!this._metadata) {
      if (this.get('table')) {
        ...
        this._metadata = new cdb.admin.CartoDBTableMetadata({id: this.get('table_id'), name: this.get('name')});
      }
    }
...
cdb.admin.Visualizations = Backbone.Collection.extend({
  ...
  url: function(method) {
    ...
    if (this.options.get('deepInsights')) {
      ...
      var useIndex = this.options.get('q') != '' || this.options.get('samples') || this.options.get('tags') != '';
