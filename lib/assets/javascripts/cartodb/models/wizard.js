...
cdb.admin.FormSchema = cdb.core.Model.extend({
      ...
      for (var k in field.form) {
        ...
        if (f.columns) {
          ...
          if (self._useAliasesInExtraFix()) {
            var namesOrAliases = self._getNamesOrAliasesFromExtra(extra);
            if (!f.value || !_.contains(namesOrAliases, f.value)) {
              f.value = namesOrAliases[0];
            }
          }
          else {
            if (!f.value) {
  ...
  _useNewModelComparison: function() {
    if (this._newComparisonDisabled === undefined) {
      this._newComparisonDisabled = _.contains(window.user_data.feature_flags, "bbg_rtr_disable_wizard_prop_comp_fix");
    }
    return !this._newComparisonDisabled;
  },
  ...
  dinamycValues: function(c) {
    ...
    if (this._useAliasesInExtraFix()) {    
      return this._getNamesOrAliasesFromExtra(v.form[k].extra);
    }
    else {
  ...
  _getNamesOrAliasesFromExtra: function(extra) {
    return _(extra).map(function (namesAndAliases) {
      // If a column has an alias, it will be stored in index 2
      // of the `namesAndAliases` schema array.  Otherwise,
      // the same position will store the original column name.
      return namesAndAliases[2];
    });
  },
